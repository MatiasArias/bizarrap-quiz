...
function answer(artists,sessionCorrect){
    ...
    while (DOR){
        sessionNumber = getRandomInt(18);
        console.log(artists[sessionNumber])
        if((sessionNumber!=sessionCorrect)&&(!(answers.includes(sessionNumber)))){
            document.getElementById(`answer${i}`).innerHTML = artists[sessionNumber];
            answers.push(sessionNumber);
            i++;
        }
        if (i==5){
            console.log(answers)
